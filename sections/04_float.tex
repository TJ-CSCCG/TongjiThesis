...
\pkg{listings} 包支持跨页显示代码段。如果遇到页眉页脚排版异常，建议使用 \pkg{minted} 包（设置 \texttt{minted=true}）。一般而言，学术写作中不建议在正文中放置大段代码，建议仅展示关键代码片段，将完整代码放入附录或以外部链接形式提供。

以下为一个跨页代码段的示例，展示长代码块场景下的排版效果：
