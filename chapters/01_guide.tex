...
本模板按学科类别对标题层级做了不同要求，请根据自身专业选择对应模板：

\begin{itemize}
  \item \textbf{理工科、经管类}：使用阿拉伯数字编号（1.1.1），其中经管类编号层级按照理工类要求；
  \item \textbf{文科（哲学、文学、历史学、法学、教育学、艺术学）}：使用汉字编号（第一章、第一节）。
\end{itemize}

\textbf{过渡期说明（26届）}：经管类专业可选用理工科模板或文科类模板。27届起，经管类统一使用理工科模板。
...
\begin{itemize}
  ...
  \item \textbf{信息说明页} —— \verb|chapters/infopage.tex|：\cs{infotype}（\texttt{thesis} 或 \texttt{design}）、\cs{infoabstract}、\cs{infothesiswords}（论文）或 \cs{infodrawings}+\cs{infowordcount}（设计）、\cs{infomaterials}，末尾调用 \cs{MakeInfoPage}。
