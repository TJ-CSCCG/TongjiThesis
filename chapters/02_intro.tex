...
本模板按照同济大学规范配置了五级标题层级。编号体系因专业类别而异，由 \verb|main.tex| 中的 \texttt{field} 选项控制，具体对应关系如\tabref{tab:heading-levels}所示。

\begin{table}[htbp]
  \centering
  \caption{标题层级与编号体系（\texttt{field} 选项）}\label{tab:heading-levels}
  \renewcommand{\arraystretch}{1.8}
  \begin{tabular}{ccllp{3.5cm}}
    \toprule
    \textbf{层级} & \textbf{\LaTeX{} 命令} & \textbf{理工科（默认）} & \textbf{文科} & \textbf{示例（理工科）} \\
    \midrule
    1 & \texttt{\textbackslash chapter}       & 1, 2, 3      & 一、二、三            & {\heiti\tjfontchapter 1\quad 绪论} \\
    2 & \texttt{\textbackslash section}       & 1.1, 1.2     & （一）（二）          & {\heiti\tjfontbody 1.1\hspace{0.5em}标题层级示例} \\
    3 & \texttt{\textbackslash subsection}    & 1.1.1, 1.1.2 & 1.\ 2.               & {\heiti\tjfontbody 1.1.1\hspace{0.5em}三级标题示例} \\
    4 & \texttt{\textbackslash subsubsection} & A, B, C      & （1）（2）（3）       & {\heiti\tjfontbody A.\hspace{0.5em}段落标题} \\
    5 & \texttt{\textbackslash paragraph}     & a, b, c      & \Circled{1}\Circled{2}\Circled{3} & {\heiti\tjfontbody a.\hspace{0.5em}子段落标题} \\
    \bottomrule
  \end{tabular}
\end{table}
...
三级标题用于在二级标题下进一步细分内容，例如在“国内外研究现状”下分别介绍国内和国外的研究进展。

\subsubsection{段落标题}

段落标题适用于需要进一步细分的场景。理工科模式下编号采用大写字母（A、B、C），文科模式下采用带括号的阿拉伯数字（1）（2）（3），符合同济大学的排版规范。

\paragraph{子段落标题}

子段落标题是最细粒度的标题层级，理工科模式下编号采用小写字母（a、b、c），文科模式下采用 \Circled{1}\Circled{2}\Circled{3} 形式。在实际论文写作中，通常不建议使用超过三级的标题层级，以保持文章结构的清晰性。
