...
\begin{table}[htbp]
  ...
  \begin{tabular}{cclll}
    ...
    4 & \texttt{\textbackslash subsubsection} & A, B, C      & （1）（2）（3）       & {\heiti\tjfontbody A.\hspace{0.5em}四级标题示例} \\
    5 & \texttt{\textbackslash paragraph}     & a, b, c      & \Circled{1}~\Circled{2}~\Circled{3} & {\heiti\tjfontbody a.\hspace{0.5em}五级标题示例} \\
    \bottomrule
  \end{tabular}
\end{table}
...
四级标题适用于需要进一步细分的场景。理工科模式下编号采用大写字母（A、B、C），文科模式下采用带括号的阿拉伯数字（1）（2）（3），符合同济大学的排版规范。

\paragraph{五级标题示例}

五级标题是最细粒度的标题层级，理工科模式下编号采用小写字母（a、b、c），文科模式下采用 \Circled{1}~\Circled{2}~\Circled{3} 形式。在实际论文写作中，通常不建议使用超过三级的标题层级，以保持文章结构的清晰性。
