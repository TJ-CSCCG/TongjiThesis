...
模板已通过 \cs{AtBeginEnvironment\{figure\}\{\textbackslash centering\}} 自动将图片居中，无需手动添加 \cs{centering}。但切勿使用 \verb|\begin{center}...\end{center}| 替代，后者会引入额外的垂直间距。
...
表格的编排建议采用国际通行的三线表\footnote{三线表以其形式简洁、功能分明、阅读方便而在科技论文中被推荐使用。三线表通常只有3条线，即顶线、底线和栏目线，没有竖线。}。可以使用 \pkg{booktabs} 宏包提供的 \cs{toprule}、\cs{midrule} 和 \cs{bottomrule} 命令来绘制三线表。同时，\pkg{longtable} 宏包可以与三线表很好地配合使用，可以排版较长的表格。按照同济大学撰写规范，三线表应「两端与页面对齐」，推荐使用 \pkg{tabularx} 宏包，将表格宽度指定为 \cs{linewidth} 即可自动撑满页宽。
要编写三线表，需要在表格的头部、底部和每一栏之间绘制水平线。其中，\cs{toprule} 绘制顶部线，\cs{midrule} 绘制中部线，\cs{bottomrule} 绘制底部线。可以按照如下方式绘制三线表：

\begin{table}[!hpt]
  \caption{一个三线表（两端与页面对齐）}
  \label{tab:firstone}
  \centering
  \begin{tabular*}{\linewidth}{@{}l@{\extracolsep{\fill}}l@{\extracolsep{\fill}}r@{}} \toprule
...
\begin{table}[!htb]
  ...
  \begin{threeparttable}[b]
    \caption{一个更大的三线表}
    \label{tab:bigtable}
    \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}llrrrrrrr} \toprule
...
\begin{ThreePartTable}
  \begin{TableNotes}
    ...
    \item[b] 另一个脚注
  \end{TableNotes}
  \begin{longtable}[c]{@{\extracolsep{\fill}}c*{6}{r}}
    \caption{实验数据}
    \label{tab:performance}                                                                                                                      \\
    \toprule
    \textbf{测试程序\tnote{b}} & \textbf{运行时间 (s)} & \textbf{同步时间 (s)}
                           & \textbf{检查点时间 (s)} & \textbf{卷回恢复时间 (s)}
                           & \textbf{进程迁移时间 (s)} & \textbf{检查点文件 (KB)} \\
    \midrule
    \endfirsthead
    \tjlongtablecont{7}
