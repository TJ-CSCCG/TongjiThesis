...
9. **Task Book / Proposal / Mid-term Report Commands** — `\taskbookperiod{startYear}{startMonth}{startDay}{endYear}{endMonth}{endDay}` (also called from `chapters/metadata.tex`, only meaningful for `doctype=taskbook`) computes the task book's week count from the date range; `\tjformtable`, `\tjformnote`, `\tjformhint`, `\tjformsection`, `\tjformsubsection`, `\tjformnotice`, `tjformnoticebody`, `\tjformlabel`, `\tjformsignature`, `\tjdateblank`, `\tjnumline`, `\tjreviewbox` are shared layout primitives for the 3 forms' subject-info table, headings, signature blocks, and review boxes — the body files call these rather than setting `\heiti`/`\zihao` by hand; `\midtermdecision{agree|disagree}` presets the mid-term report's 专业审核意见 checkbox; `\MakeTaskBook`/`\MakeProposal`/`\MakeMidterm` render each document's cover/subject table, and `\MakeDocument` dispatches to one of them based on the `doctype` option (`\ClassError` if `doctype` is still `thesis`).
